...
TaskQueue.prototype.add = function(fn, taskTimeout) {
    return new BPromise((resolve, reject) => {
        if (this.allFinished === true || this.closed === true)
        {
            reject(QUEUE_ALREADY_CLOSED_ERROR_MESSAGE);
            return;
        }
        ...
        if (this.timeoutId !== null) {
            ...
            process.nextTick(() => {
                this._nextTask(this.taskQueue[0]._task.id);
            });
        }
                ...
                id: taskId,
                    ...
                    if(newTask._task.alreadyDone === false && newTask._task.failed === false)
                    {
                        ...
                        if(newTask._task.expery === false)
                        {
                            ...
                            newTask._task.resolve(newTask._task.id);
        ...
        this._currentTasksId.push(newTask._task.id);
    });
...
TaskQueue.prototype._nextTask = function(taskId, ignore) {
    if (this._allFinished === true  || this.closed === true) {
        return;
    }
    ...
    let index2 = this._currentTasksId.indexOf(taskId);
    this._currentTasksId.splice(index2, 1);
    ...
    if (this._runningTaskLengh  < this.parallelTask && !ignore) {
        ...
        for (let i = 0 ; i < toFill && i < this.taskQueue.length;i++) {
...
TaskQueue.prototype._start = function() {
    if(this.allFinished === true || this.close === true)
        return;
...
TaskQueue.prototype._processNext = function(){
    if(this.allFinished === true || this.close === true)
        return;
    if (this.tasksListId.length == 0 ) {
        if (this.runningTaskLengh <= 0) {
            this._timeoutId = setTimeout(() => {
                this._finish();
            }, this.timeBeforeClose);
        }
    } else {
        this._nextTask(this.tasksListId[0]);
    }
};
